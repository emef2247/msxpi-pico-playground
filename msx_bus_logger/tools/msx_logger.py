...
# パケット構造: AA 55 FLAGS DROP_SEQ ADDR_LO ADDR_HI DATA SLTSL_RAW TS0 TS1 TS2 TS3
# struct フォーマット: '<2sBBHBBI'
# (magic:2s, flags:B, drop_seq:B, addr:H, data:B, sltsl_raw:B, timestamp:I)
FMT = '<2sBBHBBI'
...
def receive_loop(ser, csv_writer, raw_file, warn_drop,
    ...
    sltsl_low_count  = 0  # SLTSL Low（アサート）検出カウント
    sltsl_high_count = 0  # SLTSL High（非アサート）検出カウント
    ...
    try:
        while True:
            ...
            packet = MAGIC + rest
            try:
                magic, flags, drop_seq, addr, data, sltsl_raw, timestamp = struct.unpack(FMT, packet)
            except struct.error:
                # unpack 失敗時はスキップして再同期
                continue

            sltsl_raw_bit = sltsl_raw & 0x01  # bit0 のみ使用
            ...
            # SLTSL 生値カウント（bit0: 0=Low=アサート、1=High=非アサート）
            if sltsl_raw_bit == 0:
                sltsl_low_count += 1
            else:
                sltsl_high_count += 1
        ...
        print(f'SLTSL Low検出数  : {sltsl_low_count}')
        print(f'SLTSL High検出数 : {sltsl_high_count}')
        ...
        with open(csv_filename, 'w', newline='', encoding='utf-8') as csv_file:
            ...
            try:
                ...
                csv_writer.writerow([
                    ...
                    'rd', 'wr', 'io', 'mem', 'sltsl_raw', 'drop_latched', 'dma_lag', 'drop_seq',
                ])
